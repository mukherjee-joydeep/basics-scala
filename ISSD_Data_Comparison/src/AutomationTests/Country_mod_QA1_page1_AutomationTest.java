 ...
 *
 */
public class Country_mod_QA1_page1_AutomationTest extends AutomationTest_ISSD {
...
//		menu_page.perf_menu.click();
		...
		Assert.assertTrue(menu_page.getPage_title().getText().contains("World - Sales"),
				"clicked menu and page came are *NOT* same");

		try{
			CountrydefaultPages(menu);
...
public void CountrydefaultPages(String menu) throws InterruptedException  {
...
//		Thread.sleep(500);
		...
		String menu_page = menu.concat(" Default Page");
...
//			}
//			else
			GMdrillDownPages(molecule,menu);			
		}		
	}

	// @Test(priority = 5, dependsOnMethods = { "menuSelect" }, timeOut =
	// 290000)
	public void GMdrillDownPages(String country, String menu) throws InterruptedException  {
		 ...
		 * Invoking GM first page for further GM molecule drill down
		 */
		Country_QA1_first_page gm_fpage = PageFactory.initElements(driver, Country_QA1_first_page.class);
//		Assert.assertTrue(gm_fpage.getPage_title().getText().contains(menu), "First page is *NOT* same");

//		gm_fpage.openOptions();
//		Thread.sleep(500);

		String menu_page = menu.concat(" Page 1_"+country);
