...
import PageOModel.LoE_first_page;
 ...
 *
 */
public class LoE_page1_BuDev_ReleaseTest extends AutomationTest_ISSD {
...
//		new Object[] { "Rx Brands","Generic Molecules","Generic Suppliers" };// "OTC Categories","Generic Suppliers", "Generic Molecules"
	...
	public void GMdrillDownPages(String menu) throws InterruptedException  {
		...
		/**
		 * Invoking LoE first page for further LoE drill down
		 */
		 LoE_first_page gm_fpage = PageFactory.initElements(driver, LoE_first_page.class);
...
//		Thread.sleep(1500);
		...
		molecules.stream()
//       .filter(e->e.equalsIgnoreCase("ARCOXIA"))
		.forEach(e->{
			System.out.println(e);
			
			gm_fpage.node_drillDown(e);
			try {
				Thread.sleep(18000);
			...
			try {
				...
				softAssert.fail(menu +" "+ e+ "failed",ex);				
			}
		});
...
//		}		
	...
	public void drillPages(String menu, String molecule) throws Exception  {
		 ...
		 */
		LoE_first_page drilldownPage = PageFactory.initElements(driver, LoE_first_page.class);
		...
		Thread.sleep(1500);
		csvSaveFile(menu_page + "_" + molecule, drilldownPage.nodeValueData());
		
		List<String> loeProducts = 
				drilldownPage.getGM_brands();
		int count_loeProd=0; // added along with new code for multiple LoE prods code
		
		for (String loeProd : loeProducts) {	
			
			/**
			* New code for multiple loe Prods starts here
			*/
			if((loeProducts.size()>1)&&(count_loeProd>0)){
				
			drilldownPage.node_drillDown(molecule);
			drilldownPage = PageFactory.initElements(driver, LoE_first_page.class);
			drilldownPage.openExpanders();
			}
			count_loeProd++;
			/**
			* New code for multiple loe Prods ends here
			*/
			
			System.out.println(loeProd);
			drilldownPage.Brand_drillDown(loeProd);
			Thread.sleep(25000);
			try{
			ProddrillPages(molecule,menu, loeProd);
			}catch(Exception e){
				softAssert.fail(menu + loeProd+ "failed",e);
				continue;
			}
...
public void ProddrillPages(String moleculename,String menu, String molecule) throws Exception  {
...
//				"Drill down page is *NOT* same");
		...
		String menu_page = menu.concat(" Page 2_");
		String corpName = menu_page.concat(moleculename);
		WebElement noData=driver.findElement(By.id("bubbleChart"));
		if(noData.getText().contains("No data is available for your current selection. Please change your selection...")){
			softAssert.fail(menu + molecule+ "failed");
		...
		drilldownPage.openExpanders();
		Thread.sleep(1500);
		csvSaveFile(corpName + "_" + molecule, drilldownPage.valueData());
